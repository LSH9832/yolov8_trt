...
Inference:
    $ python download_all_models.py --weights yolov8n.pt                 # PyTorch
...
class Exporter:
    ...
    def __init__(self, config=DEFAULT_CONFIG, overrides=None):
        ...
        if overrides is None:
            overrides = {}
        self.args: DictConfig = get_config(config, overrides, strict=False)
    ...
    def _export_onnx(self, prefix=colorstr('ONNX:')):
        ...
        f = os.path.basename(str(self.file.with_suffix('.onnx')))
        dist_dir = os.path.join(self.args.dist, self.file.with_suffix("").name)
        os.makedirs(dist_dir, exist_ok=True)
        f = os.path.join(dist_dir, f).replace("\\", "/")
        self.args.f = str(Path(f).with_suffix(""))
        ...
        if "input_names" not in self.args.keys():
            self.args.input_names = input_names
        elif isinstance(self.args.input_names, str):
            self.args.input_names = [self.args.input_names]
            assert len(self.args.input_names) == len(input_names)

        if "output_names" not in self.args.keys():
            self.args.output_names = output_names
        elif isinstance(self.args.output_names, str):
            self.args.output_names = [self.args.output_names]
            assert len(self.args.output_names) == len(output_names)
