...
def get_config(config: Union[str, DictConfig], overrides: Union[str, Dict] = None, strict=True):
    """
    ...
    for key in check_config_mismatch(dict(overrides).keys(), dict(config).keys(), strict=strict):
        if not strict:
            config.__setattr__(key, overrides[key])
